...
public final class BitunixFuturesClient {
     ...
     */
    public BitunixFuturesClient(
            ApplicationConfig config,
            BitunixFuturesCandleMapper candleMapper) {
     ...
     *
     * Supports requests larger than Bitunix's
    ...
    public List<Candle> getCandles(
        ...
        while (remaining > 0) {
            ...
            int requestLimit =
                    Math.min(MAX_BITUNIX_LIMIT, remaining);
    ...
    private List<Candle> downloadCandles(
        ...
        /*
         * Bitunix returns newest -> oldest.
         * Reverse so we always return oldest -> newest.
         */
    ...
    private URI buildKlineUri(
        ...
        StringBuilder url =
                new StringBuilder(
                        String.format(
                                "%s/api/v1/futures/market/kline?symbol=%s&interval=%s&limit=%d",
    ...
    private HttpResponse<String> send(
        ...
        for (int attempt = 1; attempt <= maxRetries; attempt++) {
            ...
            try {
                ...
                throw new ExchangeException(
                        "Failed to communicate with Bitunix Futures.",
                        ex);
                ...
                System.err.printf(
                        "Bitunix request failed (attempt %d/%d). Retrying...%n",
                        attempt,
                        maxRetries);
    ...
    public Set<String> getExchangeSymbols() {
        ...
        try {
            ...
            for (JsonNode symbol : root.get("data")) {
                ...
                if (!"OPEN".equals(
                        symbol.get("symbolStatus").asText())) {
                    continue;
                }
